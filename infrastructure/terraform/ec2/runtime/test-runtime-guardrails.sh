...
require_literal 'Action   = ["ecs:DescribeTasks", "ecs:ListTasks"]' "$terraform_dir/ec2_target.tf"
...
require_literal 'OnBootSec=2min' "$runtime_dir/duducar-media-reconcile.timer"
require_literal 'OnUnitActiveSec=2min' "$runtime_dir/duducar-media-reconcile.timer"
require_literal 'refusing silent replacement' "$renderer"
require_literal 'credential_token_is_canonical()' "$renderer"
require_literal 'credential_token_is_legacy()' "$renderer"
require_literal '[ "$(wc -c < "$1")" -eq 64 ]' "$renderer"
require_literal '[ "$(wc -c < "$1")" -eq 65 ]' "$renderer"
require_literal 'head -c 64 "$existing_credential_token"' "$renderer"
require_literal "openssl rand -hex 32 | tr -d '\\n' > \"\$staging_dir/aws-credentials-token\"" "$renderer"
reject_literal 'openssl rand -hex 32 > "$staging_dir/aws-credentials-token"' "$renderer"
